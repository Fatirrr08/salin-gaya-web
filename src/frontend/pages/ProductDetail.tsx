        ...
        if (snapshot.exists()) {
          ...
          if (data.sellerUid) {
            const sellerSnap = await get(
              child(dbRef(db), `users/${data.sellerUid}`),
            );
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-16">
          <motion.div
            ...
            className="aspect-square rounded-xl overflow-hidden bg-stone-200 border border-border relative flex items-center justify-center"
          >
            ...
            <div className="mt-6 p-4 bg-card rounded-lg border border-border flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Pembayaran dilindungi oleh sistem escrow. Dana diteruskan ke
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`w-8 h-8 ${star <= newRating ? "fill-orange-400 text-orange-400" : "text-border"} hover:scale-110 transition-transform`}
                        />
                      </button>
                    ))}
                  </div>
                        ...
                        <div>
                          ...
                          <div className="flex gap-0.5 mt-0.5">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Star
                                key={i}
                                className={`w-3 h-3 ${i < review.rating ? "fill-orange-400 text-orange-400" : "text-border"}`}
                              />
                            ))}
                          </div>
                        </div>
